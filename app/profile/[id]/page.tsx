...
async function getUserProfile(userId: string) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/users/${userId}/reviews`, {
      cache: 'no-store'
    })
...
export default async function UserProfilePage({ params }: UserProfilePageProps) {
  ...
  const { language } = { language: 'en' } // TODO: Get from context
        ...
        <Card className="bg-white dark:bg-[#232323] shadow-sm border border-gray-200 dark:border-gray-700">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-[#D72638] dark:text-[#FFA5A5]">
              {language === 'de' ? 'Bewertungen von' : 'Reviews by'} {user.displayName}
            </CardTitle>
          </CardHeader>
          <CardContent>
