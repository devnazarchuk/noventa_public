...
import { Award, Calendar, Eye,Gift, Sparkles, Star } from 'lucide-react'
...
export default function SeasonalPage() {
  ...
  const seasonalProducts = getProductsByCategory('seasonal').map((product, index) => ({
    id: index + 1,
    name: product.name,
    ...
    badge: index === 0 ? 'Saison' : index === 1 ? undefined : index === 2 ? 'Neu' : undefined,
  }))
          ...
          >
            ...
            >
              ...
              <span className="text-sm font-medium text-purple-700 dark:text-purple-300">
                {language === 'de' ? 'Saisonale Spezialitäten' : 'Seasonal Specials'}
              </span>
            </motion.div>
            ...
            <p className="mx-auto mb-8 max-w-2xl text-lg text-neutral-600 sm:text-xl dark:text-neutral-300">
              {language === 'de'
                ? 'Entdecken Sie unsere besonderen saisonalen Leckereien, die nur zu bestimmten Zeiten des Jahres erhältlich sind.'
                : 'Discover our special seasonal treats that are only available at certain times of the year.'}
            </p>

            <div className="flex flex-wrap justify-center gap-4 text-sm text-neutral-600 dark:text-neutral-400">
              <div className="flex items-center gap-2">
                <Gift className="h-4 w-4" />
                <span>{language === 'de' ? 'Limitierte Zeit' : 'Limited time'}</span>
              </div>
              <div className="flex items-center gap-2">
                <Award className="h-4 w-4" />
                <span>{language === 'de' ? 'Besondere Rezepte' : 'Special recipes'}</span>
              </div>
              <div className="flex items-center gap-2">
                <Sparkles className="h-4 w-4" />
                <span>{language === 'de' ? 'Einzigartig' : 'Unique'}</span>
              </div>
            </div>
          </motion.div>
        </div>
            ...
            {seasonalProducts.map((product, index) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="mb-6 space-y-2">
                    {(language === 'de' ? product.detailsDe : product.details).map(
                      (detail, idx) => (
                        ...
                        >
                          <Star className="h-3 w-3 fill-current text-purple-500" />
                          <span>{detail}</span>
                        </div>
                      ),
                    )}
                  ...
                  <motion.div
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 px-6 py-3 font-medium text-white transition-colors duration-200 hover:bg-purple-700 cursor-pointer"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <Link href={`/products/seasonal/${product.slug}`} className="flex items-center gap-2">
        ...
        <div className="container-responsive text-center">
          ...
          >
            <h2 className="mb-4 text-3xl font-bold text-white sm:text-4xl">
              {language === 'de' ? 'Bereit für saisonale Leckereien?' : 'Ready for seasonal treats?'}
            </h2>
            <p className="mx-auto mb-8 max-w-2xl text-purple-100">
              {language === 'de'
                ? 'Besuchen Sie unsere Bäckerei und entdecken Sie unsere aktuellen saisonalen Spezialitäten.'
                : 'Visit our bakery and discover our current seasonal specialties.'}
            </p>
            <motion.button
              className="rounded-xl bg-white px-8 py-4 font-bold text-purple-600 transition-colors duration-200 hover:bg-purple-50"
