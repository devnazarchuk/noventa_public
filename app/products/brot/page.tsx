...
import { Clock, Eye, Shield, Sparkles, Star, Wheat, Zap } from 'lucide-react'
...
export default function BrotPage() {
  const { language } = useLanguageStore()
  

  const brotProducts = getProductsByCategory('brot').map((product, index) => ({
    id: index + 1,
    name: product.name,
    ...
    icon: index % 2 === 0 ? Wheat : index % 3 === 0 ? Shield : index % 4 === 0 ? Sparkles : Zap,
  }))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 dark:from-neutral-900 dark:via-neutral-800 dark:to-neutral-900">
      ...
      <section className="relative my-5 overflow-hidden py-5">
        <div className="absolute inset-0 bg-gradient-to-r from-amber-600/20 via-orange-600/20 to-yellow-600/20" />
        <div className="container-responsive relative py-16 sm:py-20 lg:py-24">
          ...
          >
            <motion.div
              className="mb-6 inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-100 px-4 py-2 dark:border-amber-700/50 dark:bg-amber-900/30"
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
            ...
            >
              ...
              <span className="text-sm font-medium text-amber-700 dark:text-amber-300">
                {language === 'de' ? 'Traditionelles Handwerk' : 'Traditional Craftsmanship'}
              </span>
            </motion.div>
            ...
            >
              ...
              >
                ...
                <span className="text-xs font-medium text-green-700 dark:text-green-300">
                  {language === 'de' ? 'Hauseigener Sauerteig' : 'House-made Sourdough'}
                </span>
              </motion.div>
              <motion.div
              ...
              >
                ...
                <span className="text-xs font-medium text-blue-700 dark:text-blue-300">
                  {language === 'de' ? 'Schamottstein gebacken' : 'Fireclay Stone Baked'}
                </span>
              </motion.div>
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
              >
                <Wheat className="h-3 w-3 text-purple-600 dark:text-purple-400" />
                <span className="text-xs font-medium text-purple-700 dark:text-purple-300">
                  {language === 'de' ? 'Bewährte Tradition' : 'Proven Tradition'}
                </span>
              </motion.div>
            </motion.div>
          </motion.div>
      ...
      <section className="container-responsive py-16 sm:py-20">
        ...
        >
          {brotProducts.map((product) => {
            const IconComponent = product.icon
            return (
              <motion.div
                key={product.id}
                className="group relative overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-lg transition-all duration-300 hover:shadow-2xl dark:border-amber-800/30 dark:bg-neutral-800"
              ...
              >
                ...
                <div className="p-6">
                  ...
                  {}
                  <motion.div
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 px-4 py-3 font-medium text-white shadow-lg transition-all duration-200 hover:from-amber-700 hover:to-orange-700 hover:shadow-xl cursor-pointer"
                    whileHover={{ scale: 1.02, y: -2 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <Link href={`/products/brot/${product.slug}`} className="flex items-center gap-2">
      ...
      <section className="container-responsive py-16 sm:py-20">
        <motion.div
          className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-amber-600 via-orange-600 to-yellow-600 p-8 text-center text-white sm:p-12"
          initial={{ opacity: 0, y: 30 }}
        ...
        >
          ...
          <div className="relative z-10">
            <h2 className="mb-4 text-3xl font-bold sm:text-4xl">
              {language === 'de' ? 'Bereit für frisches Brot?' : 'Ready for Fresh Bread?'}
            </h2>
            <p className="mx-auto mb-8 max-w-2xl text-lg text-amber-100">
              {language === 'de'
                ? 'Besuchen Sie unsere Bäckerei und erleben Sie den Duft und Geschmack von traditionell gebackenem Brot auf Schamottstein.'
                : 'Visit our bakery and experience the aroma and taste of traditionally baked bread on fireclay stone.'}
            </p>
            <motion.button
              className="mx-auto flex items-center gap-2 rounded-xl bg-white px-8 py-4 font-semibold text-amber-600 shadow-lg transition-colors duration-200 hover:bg-amber-50 hover:shadow-xl"
              whileHover={{ scale: 1.05, y: -2 }}
              whileTap={{ scale: 0.95 }}
            >
              <Wheat className="h-5 w-5" />
              {language === 'de' ? 'Bäckerei finden' : 'Find Bakery'}
            </motion.button>
          </div>
        </motion.div>
      </section>
